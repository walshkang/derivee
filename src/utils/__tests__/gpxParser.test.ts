import { parseGPXToH3 } from '../gpxParser';
...
describe('gpxParser', () => {
  it('extracts waypoints and track points and removes duplicates', () => {
    const mockGPX = `
    ...
    const result = parseGPXToH3(mockGPX);

    // Expecting 2 unique points since the first trkpt is a duplicate of wpt
    expect(result).toHaveLength(2);
    expect(result).toContain('h3_40.7128_-74.006');
    expect(result).toContain('h3_40.7129_-74.0061');
  ...
  it('throws error for invalid GPX format', () => {
    const invalidGPX = `<notgpx></notgpx>`;
    expect(() => parseGPXToH3(invalidGPX)).toThrow('Invalid GPX format: Missing root <gpx> element');
  });
});
