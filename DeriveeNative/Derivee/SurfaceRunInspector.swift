...
// MARK: - Surface Inspectable Route Protocol

/// Drives semantic and cosmetic tokens for surface-level transit (design.md §10.6).
public protocol SurfaceInspectableRoute: Sendable {
...
extension TransitModalClass: SurfaceInspectableRoute {
    public var modalClass: TransitModalClass { self }
    
    public var stopLabelNoun: String {
        switch self {
        case .ferry:
            return "Pier / Slip"
        case .bus:
    ...
    public var tracksOverWater: Bool {
        self == .ferry
    }
...
/// Surface Run Inspector sub-sheet for shared surface-level infrastructure transit:
/// City Buses, Select Bus Service (SBS), Trolleys, and Maritime Ferries.
///
/// Follows the first-principles 5-element specification (design.md §10.6):
/// 1. Hero: Identity & Imminence (Route number/name, street/pier destination, live countdown, vehicle # / vessel)
/// 2. Alert: Active Disruption Banner (inline amber warning pill, dismissible)
/// 3. Core: Stop Progression Ladder (continuous vertical route line, cross-streets or pier names, zero underground references)
...
public struct SurfaceRunInspector: View {
    ...
    private func renderStopProgressionLadder() -> some View {
        VStack(alignment: .leading, spacing: 8) {
            HStack {
                ...
                let noun = routeConfig.stopLabelNoun.uppercased()
                Text("\(stopLadder.count) \(noun)\(noun.hasSuffix("S") ? "" : "S")")
    ...
    private func renderLadderNode(stop: TrackStop, isFirst: Bool, isLast: Bool) -> some View {
        HStack(alignment: .center, spacing: 14) {
            // Progression Line Stem & Node Bullet
            VStack(spacing: 0) {
                // Upper Stem
                Rectangle()
            ...
            // Stop Information (Cross-Streets or Pier Names)
            Button {
