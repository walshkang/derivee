...
/// Zero-lock in-memory registry of subway parent station names and scheduled route endpoints.
/// Backed by official GTFS parent station IDs (`location_type = 1`), enabling sub-microsecond
/// dynamic terminal inference without synchronous database reads or lock contention.
public enum SubwayStationRegistry {
    
    // MARK: - Default Express Routes
    
    /// Routes that run express by default under standard daytime service patterns.
    public static let defaultExpressRoutes: Set<String> = [
    ...
    public static func cleanStopId(_ rawStopId: String) -> String {
        var clean = rawStopId.trimmingCharacters(in: .whitespacesAndNewlines).uppercased()
        if clean.hasPrefix("STOP_") {
            clean.removeFirst(5)
        } else if clean.hasPrefix("BUS_") {
            clean.removeFirst(4)
        }
        if let last = clean.components(separatedBy: "_").last {
            clean = last
        }
        // Strip platform direction suffix if present (e.g., L01N -> L01, 401S -> 401)
        if clean.count >= 2 && (clean.hasSuffix("N") || clean.hasSuffix("S") || clean.hasSuffix("E") || clean.hasSuffix("W")) {
            clean.removeLast()
        }
        return clean
    }
    ...
    /// Resolves official human-readable station name for a given stop ID.
    public static func resolveStationName(for stopId: String) -> String? {
        let clean = cleanStopId(stopId)
        if let name = stationsById[clean] {
            return name
        }
        if let mbtaName = mbtaTerminalsById[stopId.lowercased()] ?? mbtaTerminalsById[clean.lowercased()] {
            return mbtaName
        }
        return nil
    }
    
    // MARK: - Standard Route Endpoints (Parent Station IDs)
    
    /// Maps route IDs to their standard scheduled Northbound and Southbound terminal parent station IDs.
    public static let standardTerminals: [String: (north: Set<String>, south: Set<String>)] = [
        "1": (north: ["101"], south: ["142"]),
        "2": (north: ["201", "204"], south: ["247"]),
        "3": (north: ["301"], south: ["257"]),
        "4": (north: ["401"], south: ["250", "257"]),
        "5": (north: ["501", "204"], south: ["247"]),
        "6": (north: ["601"], south: ["640"]),
        "6X": (north: ["601"], south: ["640"]),
        "7": (north: ["701"], south: ["726"]),
        "7X": (north: ["701"], south: ["726"]),
        "A": (north: ["A02"], south: ["A65", "H11", "H15"]),
        "C": (north: ["A09"], south: ["A55"]),
        "E": (north: ["G05"], south: ["E01"]),
        "B": (north: ["D03", "114", "112"], south: ["D40"]),
        "D": (north: ["D01"], south: ["D43"]),
        "F": (north: ["F01"], south: ["D43"]),
        "FX": (north: ["F01"], south: ["D43"]),
        "M": (north: ["G08", "F12"], south: ["M01"]),
        "N": (north: ["R01"], south: ["D43"]),
        "Q": (north: ["Q05"], south: ["D43"]),
        "R": (north: ["G08"], south: ["R45"]),
        "W": (north: ["R01"], south: ["R27"]),
        "J": (north: ["G05"], south: ["M23"]),
        "Z": (north: ["G05"], south: ["M23"]),
        ...
        "BLUE": (north: ["place-wondl"], south: ["place-bomnl"]),
        "GREEN-B": (north: ["place-unsqu", "place-gover"], south: ["place-lake"]),
        "GREEN-C": (north: ["place-gover"], south: ["place-clmnl"]),
        "GREEN-D": (north: ["place-unsqu"], south: ["place-river"]),
        "GREEN-E": (north: ["place-mdftf"], south: ["place-hsmnl"]),
        "MATTAPAN": (north: ["place-asmnl"], south: ["place-matt"])
    ]
    
    /// Default fallback terminal when a trip has no remaining StopTimeUpdates.
    public static func defaultTerminal(route: String, isNorthbound: Bool) -> (stopId: String, name: String) {
        let cleanRoute = route.uppercased().trimmingCharacters(in: .whitespacesAndNewlines)
        if let config = standardTerminals[cleanRoute] {
            let candidateId = isNorthbound ? (config.north.first ?? "") : (config.south.first ?? "")
            if let name = stationsById[candidateId] ?? mbtaTerminalsById[candidateId] {
    ...
    // MARK: - NYC Subway Parent Stations (496 Official MTA Locations)
    
    public static let stationsById: [String: String] = [
